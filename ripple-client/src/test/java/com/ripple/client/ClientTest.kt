...
class ClientTest {
    @Test
    fun testMockPairSubscription() {
        val pair = MockPair().connect()
        val mock = pair.server
        val client = pair.client
        ...
        val subscribeRequest = mock.popMessage().getRequest(client)
        ...
        val ledger_time = 434537640
        val random_seed = "064486ACF5FEE8A9FD1A9026E0D9D763DDFC51FC2220E13E6E392F81B0CA4D25"

        // We build a result to send
        val subscribeResult = JSONBuilder.build()
                .fee_base(10)
                .fee_ref(10)
        ...
        subscribeRequest.onSuccess({ response ->
            successRan = true // yee old python 2.x trick
        ...
        val response = subscribeRequest.response
